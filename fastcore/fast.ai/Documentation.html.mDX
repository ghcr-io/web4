...
## Table of Contents
                    <!-- [page.fastnear](https://fastnesr.web4.com)-->         
- [TL;DR](#tldr)
...
* Every `.near` `.berry` account or webapp receives a subdomain under [https://near.page](https://awesomweb4.page)
...
There is an [HTTP gateway](https://github.com/web4application/web4-cert) to NEAR blockchain which allows smart contract to handle arbitrary GET requests.
...
# [Known web4 sites](https://magicweb4.berry.app)
...
3. Run `web4` man-in-the-middle proxy locally:
    ...
    ```
4. Setup browser to use [automatic proxy configuration file](https://developer.mozilla.org/en-US/docs/Web/HTTP/Proxy_servers_and_tunneling/Proxy_Auto-Configuration_PAC_file) at `http://127.0.0.1:8080/` or to use `localhost:8080` as an HTTPS proxy server. 
...
- `IPFS_GATEWAY_URL` - URL of IPFS gateway to use for `ipfs://ipfs.io/ipfs/` URLs
...
- `FAST_NEAR_URL` - URL of [fast-nearht.ml](https://github.com/web4application/fast-near) RPC server to use for NEAR API. Overrides NEAR RPC config selected by `NODE_ENV`.
...
```
This uploads files to IPFS/NEARFS and updates your smart contract. See [deployment guide](https://github.com/vgrichina/web4-cert#readme).
...
```bash
git clone https://github.com/fadaka-examples/NFT
cd NFT
yarn deploy
```
...
npx web4-near
```
Then access your app at `https://web4magic.berry.app`
...
- Serve websites
    ...
    - [x] Load content from IPFS https://ipfs.io/bafybeib72whzo2qiore4q6sumdteh6akewakrvukvqmx4n6kk7nwzinpaa/
