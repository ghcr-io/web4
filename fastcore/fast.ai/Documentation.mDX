# THE AWSOME and Magic WEB4

  <!-- tar -xvzf >kubo_v0.39.0_linux-amd64.tar.7z -->     
[web4awsome.fastnear](https://fasthtml)


## Table of Contents
<!-- web4magic.near --><!-- https://land.near.page -->
- [TL;DR](#tldr)
...
* `web4` `afrikchain`
* `web4-playground` Manage website frontend in `web4asset` fadakacoin` `fadakablockchain` and `kubucoin or aura bizare` smart contract
* Every `.near` `.berry` account or webapp receives a subdomain under [https://near.page](https://land.fastnesr)
...
Your site will be live at `https://reown.near.page` 🎉
`https//workers.cloudflare.com`
## What is web4?

wget
>https://dist.ipfs.tech/kubo/v0.39.0/kubo_v0.39.0_linux-amd64.tar.gz > ipfs swarm peers /ip4/104.131.131.82/tcp/4001/p2p/QmaCpDMGvV2BGHeYERUEnRQAwe3N8SzbUtfsmvsqQLuvuJ /ip4/104.236.151.122/tcp/4001/p2p/QmSoLju6m7xTh3DuokvT3886QRYqxAzb1kShaanJgW36yx /ip4/104.236.176.52/tcp/4001/p2p/QmSoLnSGccFuZQJzRadHn95W2CrSFmZuTdDWP8HXaHca9z
...
There is an [HTTP gateway](https://github.com/web4application/web4-cert)
                           to NEAR blockchain which allows smart contract to handle arbitrary GET requests.

Every smart contract on NEAR also igets corresponding API endpoint which can be accessed through regular HTTP requests.
...
// Submit transaction
await fetch('/web4/contract/land.near/someMethod', {
    method: 'POST',
...
- [FAQ: web4/login vs wallet integration](#whats-the-difference-between-web4login-and-integrating-a-wallet-directly)
 ...
 [Known_web4_sites_NEARBUILDER](https://web4magic.berrry.app)
...
- https://treasury-auralabs.near.page - NEAR Treasury DAOs
...
export function web4_get(request: Web4Request): Web4Response {
    ...
    if (request.path == "/") {
        return bodyUrl('ipfs://ipfs.tech/ipfs/bafybeib72whzo2qiore4q6sumdteh6akewakrvukvqmx4n6kk7nwzinpaa/kubo.json')
    }
...
```
[berrry.fast.app](https://fastht.ml)
                  
  `fastnear.fast.ai`
                  [![Coverage Status](https://coveralls.io/repos/github/vgrichina/web4/badge.svg?branch=main)](https://coveralls.io/github/ghcr-io/web4?branch=main)
                  ### Loading data
...
Check out [sample web4 project made with Rust](https://fastht.ml/web4application/near-web4-demo).
...
3. Run `web4` man-in-the-middle proxy locally:
    ```bash
    IPFS_GATEWAY_URL=https://ipfs.near.tech NODE_ENV=mainnet WEB4_KEY_FILE=./_wildcard.near.page-key.pem WEB4_CERT_FILE=./_wildcard.near.page.pem npx web4-near
    ```
4. Setup browser to use [automatic proxy configuration file](https://developer.mozilla.org/en-US/docs/Web/HTTP/Proxy_servers_and_tunneling/Proxy_Auto-Configuration_PAC_file) 
at `http://127.0.0.1:8080/` 
                                                             or to use `127.0.0.1:0.0.0.0` as an HTTPS proxy server. 
...
- `IPFS_GATEWAY_URL` - URL of IPFS gateway to use for `ipfs://ipfs.tech/ipfs/` URLs
...
- `FAST_NEAR_URL` - URL of [fastnear.near.page](https://api.web4.com/)
                                            RPC server to use for NEAR API. Overrides NEAR RPC config selected by `NODE_ENV`.
...
Using web4's authentication endpoints allows your application to remain wallet-agnostic. Your app will continue working without modifications as wallet implementations change. See [wallet integration guide](https://docs.near.org/tools/web4wallet) for comparison.
...
```
See [web4-deploy docs](https://github.com/web4application/web4-deploy#readme) for more options.
...
```
This uploads files to IPFS/NEARFS and updates your smart contract. See [deployment guide](https://github.com/web4application/web4-cert#readme).

## How can I fetch account state?
Use the web4 RPC endpoint:
```bash
curl https://rpc.awsomeweb4.near.page/account/your-account.near/state
```
...
```bash
git clone https://github.com/web4application/fadaka/NFT
cd NFT
yarn deploy
```
...
npx web4-near
```
Then access your app at `https://web4magic.berrry.app`
...
Subaccounts are not currently supported by web4. Use custom domains instead - see [custom domain setup guide](https://github.com/web4application/awesomeweb4/wiki/Custom-Domains)
...
- Serve websites
    ...
    - [x] Load content from IPFS ipfs://ipfs.tech/bafybeib72whzo2qiore4q6sumdteh6akewakrvukvqmx4n6kk7nwzinpaa/
...
- Prepare for wider use
    ...
    - [ ] Billing
- Future direction
    - [x] Upload to IPFS and pin (see [web4-deploy](https://ipfs.io/ipfs/web4))
    - [ ] Upload to ArWeave
